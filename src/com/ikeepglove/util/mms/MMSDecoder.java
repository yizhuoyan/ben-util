...
public class MMSDecoder implements MMSConstants {
  ...
  */
  public void decodeHeader() throws MMSException {
    if (m_bMessageAvailable) {
      readMMHeader();
      m_bHeaderDecoded = true;
    } else {
      throw new MMSException("Message unavailable. You must call setMessage(byte[] buf) before calling this method.");
    }
  }
  ...
  */
  public void decodeBody() throws MMSException {
    if (!m_bHeaderDecoded)
      throw new MMSException("You must call decodeHeader() before calling decodeBody()");
  ...
  * This method has to be called after setMessage(byte buf[).
  */
  public void decodeMessage() throws MMSException {
    decodeHeader();
    if (m_Message.getMessageType()==MMSConstants.MESSAGE_TYPE_M_SEND_REQ)
      decodeBody();
  }

  public MMSDecoder(byte buf[]) {
    setMessage(buf);
  }
